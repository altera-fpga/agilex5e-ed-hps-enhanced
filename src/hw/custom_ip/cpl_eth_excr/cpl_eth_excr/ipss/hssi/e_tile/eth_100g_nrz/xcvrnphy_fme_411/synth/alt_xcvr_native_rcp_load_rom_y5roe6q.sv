...
module alt_xcvr_native_rcp_load_rom_y5roe6q
...
#(
  ...
  localparam rom_addr_width  = altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(alt_xcvr_native_rcp_load_rom_params_y5roe6q::rom_depth-1); //Calculate address width

  wire [31:0]                rom_addr_long;
  wire [ROM_ADD_WIDTH-1:0]   rom_addr;

  assign rom_addr_long = get_sum(rcp_load_rcp_sel, alt_xcvr_native_rcp_load_rom_params_y5roe6q::ical_depth_list)  + addr; //Calculate rom address using sum of preceding configuration depths and address within current configuration
  assign rom_addr      = rom_addr_long[ROM_ADD_WIDTH-1:0];        //Truncate rom address to correct width

  (* romstyle = rcfg_rom_style *) reg [ROM_DATA_WIDTH-1:0] rom [0:alt_xcvr_native_rcp_load_rom_params_y5roe6q::rom_depth-1]; //Inferred rom

  initial begin
    rom = alt_xcvr_native_rcp_load_rom_params_y5roe6q::config_rom;
  end
